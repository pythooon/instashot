...
final class SeedDatabaseCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $seedUsernames = array_column($usersData, 'username');
        $this->resetSeedData($seedUsernames, $io);
     ...
     */
    private function resetSeedData(array $usernames, SymfonyStyle $io): void
    {
        if ($usernames === []) {
            return;
        }

        $io->comment('Replacing any existing rows for the built-in demo accounts (safe to run multiple times).');

        $this->entityManager->createQuery(
            'DELETE FROM App\Like\Entity\Like l '
            . 'WHERE l.user IN (SELECT u.id FROM App\Auth\Entity\User u WHERE u.username IN (:names))'
        )->setParameter('names', $usernames)->execute();

        $this->entityManager->createQuery(
            'DELETE FROM App\Photo\Entity\Photo p '
            . 'WHERE p.user IN (SELECT u.id FROM App\Auth\Entity\User u WHERE u.username IN (:names))'
        )->setParameter('names', $usernames)->execute();

        $this->entityManager->createQuery(
            'DELETE FROM App\Auth\Entity\AuthToken t '
            . 'WHERE t.user IN (SELECT u.id FROM App\Auth\Entity\User u WHERE u.username IN (:names))'
        )->setParameter('names', $usernames)->execute();

        $this->entityManager->createQuery(
            'DELETE FROM App\Auth\Entity\User u WHERE u.username IN (:names)'
        )->setParameter('names', $usernames)->execute();
